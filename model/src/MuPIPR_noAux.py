...
id2seq_file = '/workspace/PPI-Binding/hotspot/processed/seq.txt'

# ds_file, label_index, rst_file, use_emb, hiddem_dim
# ds_file = '/workspace/PPI-Binding/iSee-master/processed/4g_scores.txt'
ds = '/workspace/PPI-Binding/BindProfX/processed/bpx.ddg.txt'
label_index = 4
...
total_mae = 0.
total_cov = 0.
fp2 = open('records/'+rst_file[rst_file.rfind('/')+1:], 'w')
...
for train, test in train_test:
    ...
    merge_model.fit([seq_tensor[seq_index1[train]], seq_tensor[seq_index2[train]], seq_tensor[seq_index3[train]], seq_tensor[seq_index4[train]]], [score_labels[train,0]], batch_size=batch_size1, epochs=n_epochs, verbose = 1)

    ddG_pred = merge_model.predict([seq_tensor[seq_index1[test]], seq_tensor[seq_index2[test]], seq_tensor[seq_index3[test]], seq_tensor[seq_index4[test]]])
    # only takes m1, m2 => predicted g2
    # Changed to predict both Gs
    # pred_Gm = merge_model.predict([seq_tensor[seq_index3[test]], seq_tensor[seq_index4[test]]])

    # pred_Gw = merge_model.predict([seq_tensor[seq_index1[test]], seq_tensor[seq_index2[test]]])

    # pred = scale_back(np.ndarray.flatten(pred_Gm)) - scale_back(score_labels[test,0])
    # pred = scale_back(np.ndarray.flatten(pred_Gm)) - scale_back(np.ndarray.flatten(pred_Gw))
    # ddG_pred = scale_back(pred[1]) - scale_back(pred[0])
    # ddG_pred = pred[1] - pred[0]

    
    # dG_pred0 = scale_back(pred[0])
    # dG_pred1 = scale_back(pred[1])

    # print(np.ndarray.flatten(pred_Gm), pred)
    # print(score_labels[test,0].shape)

    # !!!!! Don't use this scale back!!! Update under each loop. Need to rename
    # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    # ddG_pred = scale_back(ddG_pred)
    # score_labels = scale_back(score_labels)
    ...
    for i in range(len(score_labels[test])):
        ...
        # v * (all_max - all_min) + all_min
        ddG_label_i = (all_max - all_min)*score_labels[test,0][i] + all_min
        ddG_pred_i = (all_max - all_min)*ddG_pred[i] + all_min

        # diff = abs(score_labels[test,num_scores-1][i] - ddG_pred[i])
    ...
    for i in range(len(test)):
        ...
        # fp2.write(str(raw_ids[test[i]][sid1_index]) + '\t' + str(raw_ids[test[i]][sid2_index])  + '\t' + str(raw_ids[test[i]][sid3_index])  + '\t' + str(raw_ids[test[i]][sid4_index])+ '\t' + str(np.ndarray.flatten(ddG_pred)[i]) + '\t' + str(score_labels[test[i], 0]) + '\n') 
        ...
        + '\t' + str(scale_back(np.ndarray.flatten(ddG_pred)[i])) + '\t' + str(scale_back(score_labels[test[i],0])) + '\t' + str(np.ndarray.flatten(ddG_pred)[i]) + '\n')
